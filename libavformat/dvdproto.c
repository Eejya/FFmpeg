...
static int dvd_open(URLContext *h, const char *filename, int flags)
{
	...
	if( dvdnav_menu_call( dvd->nav_data, DVD_MENU_Title ) != DVDNAV_STATUS_OK )
		{
			av_log(h,0,"Could not access the title's menu\n");
		}
	if( dvdnav_menu_call( dvd->nav_data, DVD_MENU_Root ) != DVDNAV_STATUS_OK )
		{
			av_log(h,0,"Could set it to the root menu\n");
		}
...
while(!finished)
{
    ...
    case DVDNAV_NAV_PACKET:
		...
		av_log(h,0,"No. of buttons are :%d\n",pci->hli.hl_gi.btn_ns);
		dvdnav_get_current_nav_dsi(dvd->nav_data);
		if(pci->hli.hl_gi.btn_ns > 0) {
		int button = 0;
		
		av_log(h,0,"Found %i DVD menu buttons...\n", pci->hli.hl_gi.btn_ns);
		btni_t *btni = &(pci->hli.btnit[button]);
		
		for (button = 0; button < pci->hli.hl_gi.btn_ns; button++) {
			btni_t *btni = &(pci->hli.btnit[button]);
			av_log(h,0,"Button %i top-left @ (%i,%i), bottom-right @ (%i,%i)\n",button + 1, 
